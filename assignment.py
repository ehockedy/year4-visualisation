...
def load_binary_values_2(files, num_bytes=2):
    pixel_vals = []
    for img in files:
        with open("dataset/" + img, "rb") as f:
            byte = 0
            while byte != b"":
                # Do stuff with byte
                byte = f.read(num_bytes)  # Read in 2 bytes = 16 bits - ASSIGNMENT SAYS 8 BITS BUT DATA SET IS 16
...
def visualise_marching_cubes(points, num_slices, width=256, height=256, iso_value_1=0.1, iso_value_2=0.33, do_smoothing=False, biggest_region=True):
    ...
    # Link the callback function. This allows for custom functions upon interaction with the render window
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        xpixs = int(sys.argv[1])
        ypixs = int(sys.argv[2])
        no_imgs = int(sys.argv[3])
        print(xpixs, ypixs, no_imgs, len(sys.argv[4:]))
        a = load_binary_values_2(sys.argv[4:])
        visualise_marching_cubes(a, len(sys.argv[4:])-1, xpixs, ypixs, 0.25, 0.5, True, True)
